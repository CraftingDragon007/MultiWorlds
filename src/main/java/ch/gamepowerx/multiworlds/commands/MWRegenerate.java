...
public class MWRegenerate implements CommandExecutor {
    private  List<WorldCreator> creators = new ArrayList<>();
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        Bukkit.broadcastMessage("§eDie Welten werden regeneriert, du wirst gekickt!");
        MultiWorlds.isInRegeneration = true;
        for(Player player : Bukkit.getOnlinePlayers()){
            player.kickPlayer("§cWorlds are being regenerated!");
        }
